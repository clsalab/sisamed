...
const { citaModel } = require("../models");
...
const createItem = async (req, res) => {
  try {
    ...
    const user = req.user;
    const data = await citaModel.create(body);
    res.send({ data, user });
  } catch (e) {
...
const updateItem = async (req, res) => {
  try {
    ...
    const user = req.user;
    const data = await citaModel.findByIdAndUpdate(id, body, { new: true });
    res.send({ data, user });
  } catch (e) {
...
const deleteItem = async (req, res) => {
  try {
    ...
    const { id } = req;
    const user = req.user;
    const data = await citaModel.delete({_id:id});
    res.send({ data, user });
  } catch (e) {
